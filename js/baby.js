...
var babyObj = function () {
    ...
    this.babyTailTimer = 0;//鱼尾计时器
    this.babyTailCount = 0;//记录当前图片序号变量
...
babyObj.prototype.draw = function () {
    ...
    //计算坐标差，实现转身; delta angle,Math.atan2(y,x)正切; 跟着大鱼转身
    var deltaY = mom.y - this.y;//小鱼跟着大鱼的坐标差
    var deltaX = mom.x - this.x;
    var beta = Math.atan2(deltaY, deltaX) + Math.PI; //-PI,PI
    ...
    if (this.babyEyeTimer > this.babyEyeInterval) {
        ...
        if (this.babyEyeCount == 0) {//如果小鱼睁着眼睛
            this.babyEyeInterval = Math.random() * 1500 + 2000; //[0,1),后面单位ms。睁眼睛时间为1500到3500毫秒之间
        }else{
